...
class TaskHandler {
  ...
  createTaskElement(task) {
    ...
    taskElement.addEventListener("click", () =>
      this.app.openTaskModal(task.columnId, task.id)
    );
  ...
  setupTaskDragEvents() {
    ...
    document.querySelectorAll(".tasks").forEach((container) => {
      container.addEventListener("dragenter", (e) =>
        this.taskContainerDragEnter(e)
      );
      container.addEventListener("dragleave", (e) =>
        this.taskContainerDragLeave(e)
      );
  ...
  taskContainerDragEnter(e) {
    ...
    if (this.draggedTaskId) {
      ...
      if (
        e.currentTarget.dataset.columnId === "unassigned" &&
  ...
  handleTaskDrop(container, taskId) {
    ...
    console.log(
      `Moving task ${taskId} to ${columnId === "unassigned" ? "unassigned" : columnId} (from ${task.columnId === null ? "unassigned" : task.columnId})`
    );

    // Check if task is already in this column
    // Always allow moving FROM unassigned TO another column
    if (
      (columnId === "unassigned" && task.columnId === null) ||
      (columnId !== "unassigned" &&
    ...
    if (this.app.authManager && this.app.authManager.isAuthenticated) {
      ...
      if (
        this.app.authManager.ws &&
        this.app.authManager.ws.readyState === WebSocket.OPEN
      ) {
        const message = {
          type: "taskMove",
          data: {
  ...
  markTaskAsDeleted(taskId) {
    // Find the task
    const taskIndex = this.app.data.tasks.findIndex(
      (task) => task.id === taskId
    );
    if (taskIndex !== -1) {
